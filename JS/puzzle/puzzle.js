window.addEventListener('DOMContentLoaded', () =>{
                ...
                let scrWidth = leftPane.offsetWidth;
                let scrHeight = leftPane.offsetHeight;
                for (let i = 0; i < 1000; i++){
                    let randX = Math.round(Math.random() * (width - 1));
                    let randY = Math.round(Math.random() * (height - 1));
                    let randTile = tiles[randY][randX];

                    let posLeft = Math.round(Math.random()*(scrWidth - size));
                    let posTop = Math.round(Math.random()*(scrHeight - size));
                        ...
                        let checkOver = e =>{
                            ...
                            if (t.classList.contains('puzzle-cell')){
                                if (t.x === target.x && t.y === target.y){
                                    ...
                                    t.append(target);
                                    fullCells.push(t);
                                    if (fullCells.length === width * height){
                                        let winText = document.createElement('p');
                                        winText.className = 'win'
                                        winText.textContent = 'Вы собрали паззл!'
                                        leftPane.appendChild(winText)
                                    }
                                    checkUp();
